<template>
  ...
  >
    <div v-if="show" class="absolute inset-0 bg-black/50 flex items-center justify-center z-10">
      <div 
        class="bg-white dark:bg-gray-800 rounded-xl p-6 m-4 max-w-sm w-full shadow-xl"
        :class="modalClass"
      >
        <!-- Icon for warning variant -->
        <div v-if="icon === 'warning'" class="flex items-center gap-3 mb-4">
        ...
        <!-- Standard title -->
        <h3 v-else class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
          {{ title }}
        </h3>
